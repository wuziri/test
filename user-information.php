...
$sql=$db->query("select * from user limit ".($Page-1)*$pagesize.",".$pagesize."");
...
<center><table>
        <tr>
            <th width="512" align="left" >用户名</th>
            <th width="512" align="left" >电话</th>
            <th width="512" align="left" >邮箱</th>
            <th width="167" > 操作 </th>
        </tr>
    <?php
    while($row=$sql->fetch()){
    ...
    <tr>
        <td>
            <a href="user-buyshop.php"> <?= $row['username']?></a>
        </td>
        <td><?= $row['tel']?></td><td><?= $row['email']?></td>
        <td><a href="delete-user.php?id=<?=$row['id'];?>">删除</a> </td>
    </tr>
    <?php };?>
</table></center>
            <?php
            $_SESSION['c']=$row['username'];
            if($Page==1)
                echo "第一页  上一页";
